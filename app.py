...
if os.path.exists(vacantes_file):
    ...
    backup_name = f"backups/vacantes_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    shutil.copy(vacantes_file, backup_name)
# --- RESPALDO AUTOMÁTICO ---
if os.path.exists(vacantes_file):
    import shutil
    from datetime import datetime
    backup_name = f"vacantes_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    shutil.copy(vacantes_file, backup_name)
else:
...
with st.expander("🔎 Buscar establecimiento"):
    ...
    if filtro:
        ...
        if not resultado.empty:
            ...
            seleccionado = resultado.iloc[0]
            centrar_mapa = (seleccionado["LAT"], seleccionado["LONG"])
            establecimiento_destacado = seleccionado["NOM_ESTABLEC"]
        else:
...
if 'centrar_mapa' in locals():
    m = folium.Map(location=centrar_mapa, zoom_start=15)
else:
...
if 'establecimiento_destacado' in locals() and fila["NOM_ESTABLEC"] == establecimiento_destacado:
